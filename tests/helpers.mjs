...
const SOURCE_DATASTORE_MODULE = path.resolve(FIXTURES_DIR, '..', '..', 'dataStore.mjs');
const SOURCE_DATASTORE_CONSTANTS = path.resolve(FIXTURES_DIR, '..', '..', 'datastore.mjs');
...
export async function createWebAdminSandbox() {
    ...
    await fs.cp(SOURCE_DATASTORE_MODULE, path.join(agentRoot, 'dataStore.mjs'));
    await fs.cp(SOURCE_DATASTORE_CONSTANTS, path.join(agentRoot, 'datastore.mjs'));
